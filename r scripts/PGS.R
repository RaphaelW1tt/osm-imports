...
count <- read.delim("../files india/india/PGS/unique contributions/count.txt",sep =" ", header = FALSE, dec =".")
freq <- read.delim("../files india/india/PGS/unique contributions/unique freq.txt",sep =" ", header = FALSE, dec =".")

count_data <- count$V2
freq_data <- freq$V2

count_ts <- ts(count_data, start = decimal_date(as.Date("2007-12-01")), frequency = 365)
freq_ts <- ts(freq_data, start = decimal_date(as.Date("2007-12-01")), frequency = 365)
